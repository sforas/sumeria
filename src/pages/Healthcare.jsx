...
export default function Healthcare() {
  ...
  const payload = {
    ...
    alternate_start: newMed.frequency === 'alternate' && newMed.alternate_start
      ? newMed.alternate_start
      : null
  }
  const { data, error } = await supabase.from('medicines').insert(payload).select().single()
  console.log('Medicine result:', data, error)
  if (data) setMedicines([...medicines, data].sort((a, b) => a.time?.localeCompare(b.time)))
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '14px' }}>
            <div style={{ background: 'var(--surf)', border: '0.5px solid var(--border)', borderRadius: '10px', padding: '11px 13px' }}>
              ...
              <div style={{ fontSize: '20px', fontWeight: 500, color: takenCount === todayMeds.length && todayMeds.length > 0 ? 'var(--fit)' : 'var(--health)' }}>
                {takenCount}/{todayMeds.length}
              </div>
              <div style={{ height: '3px', background: 'var(--surf3)', borderRadius: '2px', marginTop: '6px', overflow: 'hidden' }}>
                <div style={{
                  width: `${todayMeds.length > 0 ? takenCount / todayMeds.length * 100 : 0}%`,
                  height: '100%', background: 'var(--health)', borderRadius: '2px', transition: 'width .3s'
                }} />
              </div>
            </div>
            <div style={{ background: 'var(--surf)', border: '0.5px solid var(--border)', borderRadius: '10px', padding: '11px 13px' }}>
          ...
          <div style={{ fontSize: '10px', fontWeight: 500, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.6px', marginBottom: '8px' }}>
            Today's medicines {todayMeds.length < medicines.length ? `(${medicines.length - todayMeds.length} not scheduled today)` : ''}
          </div>
          ...
          {todayMeds.map(med => (
            <div key={med.id} style={{
              ...
              opacity: medLog[med.id] ? 0.6 : 1
            }}>
              <div style={{
                width: '32px', height: '32px', borderRadius: '7px',
                background: '#160610', display: 'flex', alignItems: 'center',
                justifyContent: 'center', flexShrink: 0, fontSize: '16px'
              }}>💊</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '13px', fontWeight: 500, marginBottom: '1px' }}>
                  {med.name}
                  {med.frequency === 'alternate' && (
                    <span style={{ fontSize: '10px', color: 'var(--acc)', marginLeft: '6px' }}>every other day</span>
                  )}
                </div>
                <div style={{ fontSize: '11px', color: 'var(--muted2)' }}>
                  {med.dose} · {med.time}{med.with_food ? ' · with food' : ''}
                </div>
              </div>
              <div
                ...
                style={{
                  ...
                  border: medLog[med.id] ? 'none' : '1.5px solid var(--border)',
                  background: medLog[med.id] ? 'var(--health)' : 'none',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  cursor: 'pointer', fontSize: '12px', color: '#fff', flexShrink: 0
                }}
              >{medLog[med.id] ? '✓' : ''}</div>
          ...
          {showAddMed ? (
            <div style={{
              background: 'var(--surf)', border: '0.5px solid var(--border)',
              borderRadius: '10px', padding: '13px', marginTop: '6px', marginBottom: '14px'
            }}>
              <div style={{ fontSize: '13px', fontWeight: 500, marginBottom: '10px' }}>Add medicine</div>
              <input placeholder="Medicine name" value={newMed.name}
              ...
              <input placeholder="Dose (e.g. 1 tablet)" value={newMed.dose}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                ...
                <div>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', marginBottom: '4px' }}>Notify before</div>
                  <input type="number" placeholder="Minutes" value={newMed.notify_before_min}
                    onChange={e => setNewMed({ ...newMed, notify_before_min: e.target.value })}
                    style={{ width: '100%', background: 'var(--surf3)', border: '0.5px solid var(--border)', borderRadius: '7px', color: 'var(--text)', fontSize: '13px', padding: '9px 11px', outline: 'none' }} />
                </div>
              </div>
              ...
              {newMed.frequency === 'alternate' && (
                <div style={{ marginBottom: '8px' }}>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', marginBottom: '4px' }}>First day you took it</div>
                  <input type="date" value={newMed.alternate_start}
                    onChange={e => setNewMed({ ...newMed, alternate_start: e.target.value })}
                    style={{ width: '100%', background: 'var(--surf3)', border: '0.5px solid var(--border)', borderRadius: '7px', color: 'var(--text)', fontSize: '13px', padding: '9px 11px', outline: 'none' }} />
                </div>
              )}
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                <input type="checkbox" id="withfood" checked={newMed.with_food}
                  onChange={e => setNewMed({ ...newMed, with_food: e.target.checked })}
                  style={{ width: '16px', height: '16px', cursor: 'pointer' }} />
                <label htmlFor="withfood" style={{ fontSize: '13px', cursor: 'pointer' }}>Take with food</label>
              </div>
              <div style={{ display: 'flex', gap: '7px' }}>
                <button onClick={addMedicine} style={{
                  flex: 1, background: 'var(--health)', border: 'none', borderRadius: '7px',
                  color: '#fff', fontSize: '13px', padding: '9px', cursor: 'pointer', fontWeight: 500
                }}>Save</button>
                <button onClick={() => setShowAddMed(false)} style={{
            ...
            <button onClick={() => setShowAddMed(true)} style={{
              ...
              color: 'var(--muted)', cursor: 'pointer', fontSize: '12px', marginTop: '4px', marginBottom: '14px'
            }}>+ Add medicine</button>
          )}
          ...
          {sleepLog.slice(0, 5).map(s => {
            ...
            return (
              <div key={s.id} style={{
                ...
                padding: '9px 12px', background: 'var(--surf)', border: '0.5px solid var(--border)',
                borderRadius: '8px', marginBottom: '5px'
              }}>
                <span style={{ fontSize: '11px', color: 'var(--muted)' }}>{s.date}</span>
                <span style={{ fontSize: '12px' }}>{s.sleep_time} → {s.wake_time}</span>
                <span style={{ fontSize: '12px', fontWeight: 500, color: dur && dur.total >= 420 ? 'var(--fit)' : 'var(--xp)' }}>
                  {dur?.label || '—'}
                </span>
                <button onClick={() => deleteSleep(s.id)} style={{
          ...
          {showLogSleep ? (
            <div style={{
              ...
              <div style={{ fontSize: '13px', fontWeight: 500, marginBottom: '10px' }}>Log sleep</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
                <div>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', marginBottom: '4px' }}>Went to sleep</div>
                  <input type="time" value={newSleep.sleep_time}
                    onChange={e => setNewSleep({ ...newSleep, sleep_time: e.target.value })}
                    style={{ width: '100%', background: 'var(--surf3)', border: '0.5px solid var(--border)', borderRadius: '7px', color: 'var(--text)', fontSize: '13px', padding: '9px 11px', outline: 'none' }} />
                </div>
                <div>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', marginBottom: '4px' }}>Woke up</div>
                  <input type="time" value={newSleep.wake_time}
                    onChange={e => setNewSleep({ ...newSleep, wake_time: e.target.value })}
                    style={{ width: '100%', background: 'var(--surf3)', border: '0.5px solid var(--border)', borderRadius: '7px', color: 'var(--text)', fontSize: '13px', padding: '9px 11px', outline: 'none' }} />
                </div>
              </div>
              {newSleep.sleep_time && newSleep.wake_time && (
                <div style={{ fontSize: '13px', color: 'var(--fit)', marginBottom: '10px', textAlign: 'center' }}>
                  Duration: {calcDuration(newSleep.sleep_time, newSleep.wake_time)?.label}
                </div>
              )}
              <div style={{ display: 'flex', gap: '7px' }}>
                ...
                <button onClick={() => setShowLogSleep(false)} style={{
                  background: 'var(--surf3)', border: '0.5px solid var(--border)',
                  borderRadius: '7px', color: 'var(--muted)', fontSize: '13px',
                  padding: '9px 14px', cursor: 'pointer'
                }}>Cancel</button>
            ...
            <button onClick={() => setShowLogSleep(true)} style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px',
              width: '100%', padding: '9px', background: 'none',
              border: '0.5px dashed var(--border)', borderRadius: '8px',
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px', marginBottom: '12px' }}>
            {[
              ...
              { label: 'Medicines', value: `${medicines.length}`, color: 'var(--health)', sub: 'configured' },
            ].map((k, i) => (
          ...
          {weekSleep.map(s => {
            ...
            return (
              <div key={s.id} style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '8px 12px', background: 'var(--surf)', border: '0.5px solid var(--border)',

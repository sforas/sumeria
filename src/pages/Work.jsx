        ...
        <>
          ...
          {applications.map(app => (
            <div key={app.id} style={{
              ...
              background: 'var(--surf)', border: '0.5px solid var(--border)',
              borderRadius: '8px', marginBottom: '6px', alignItems: 'flex-start'
            }}>
              <div style={{
                width: '8px', height: '8px', borderRadius: '50%',
                background: statusColor(app.status), marginTop: '5px', flexShrink: 0
              }} />
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                  ...
                  <button
                    ...
                    style={{
                      padding: '2px 8px', borderRadius: '4px', fontSize: '10px',
                      cursor: 'pointer', border: '0.5px solid var(--border)',
                      background: 'none', color: 'var(--danger)', marginLeft: 'auto'
                    }}
                  >Delete</button>
                </div>
              </div>
          ...
          {showAdd ? (
            <div style={{
              ...
              <div style={{ fontSize: '13px', fontWeight: 500, marginBottom: '10px' }}>New application</div>
              <input
                autoFocus
                placeholder="Company"
              ...
              <div style={{ display: 'flex', gap: '7px' }}>
                <button onClick={addApplication} style={{
                  flex: 1, background: 'var(--work)', border: 'none', borderRadius: '7px',
      ...
      {view === 'week' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
            {[
              { label: 'Apps this week', value: applications.filter(a => {
                const d = new Date(a.date)
                const weekAgo = new Date()
                weekAgo.setDate(weekAgo.getDate() - 7)
                return d >= weekAgo
              }).length, color: 'var(--work)' },
              { label: 'Interviews', value: interviews.length, color: 'var(--xp)' },
            ].map((k, i) => (
              <div key={i} style={{ background: 'var(--surf)', border: '0.5px solid var(--border)', borderRadius: '10px', padding: '11px 13px' }}>
                <div style={{ fontSize: '10px', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: '4px' }}>{k.label}</div>

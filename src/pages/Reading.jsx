...
export default function Reading() {
  ...
  async function updatePages(book) {
    ...
    const { data } = await supabase
      .from('books')
      .update({ pages_read: updatedPages, status })
      .eq('id', book.id)
      .select().single()
    ...
    if (status === 'finished') {
      await supabase.from('xp_log').insert({ amount: 200, reason: 'Book finished!', date: today() })
    }
  }
            ...
            <div key={book.id} style={{
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                <div style={{
                  ...
                  background: 'var(--surf3)', flexShrink: 0,
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--muted2)', marginBottom: '6px' }}>
                    {book.author} · {book.format === 'physical' ? 'Physical' : 'Kindle'}
                  </div>
                  ...
                  <div style={{ height: '3px', background: 'var(--surf3)', borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{
                      width: `${book.total_pages > 0 ? book.pages_read / book.total_pages * 100 : 0}%`,
                      height: '100%', background: 'var(--read)', borderRadius: '2px', transition: 'width .3s'
                    }} />
                  </div>
                </div>
              ...
              {showLogPages === book.id ? (
                <div style={{ display: 'flex', gap: '7px', marginTop: '10px' }}>
                  <input
                    autoFocus
                    placeholder={`Current page (max ${book.total_pages})`}
                    type="number"
                    value={newPages}
                  ...
                  />
                  <button onClick={() => updatePages(book)} style={{
                    background: 'var(--read)', border: 'none', borderRadius: '7px',
                    color: '#000', fontSize: '12px', padding: '8px 14px', cursor: 'pointer', fontWeight: 500
                  }}>Save</button>
                  <button onClick={() => setShowLogPages(null)} style={{
                ...
                <button onClick={() => setShowLogPages(book.id)} style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px',
                  width: '100%', padding: '7px', background: 'none',
                  border: '0.5px dashed var(--border)', borderRadius: '7px',
                  color: 'var(--muted)', cursor: 'pointer', fontSize: '11px', marginTop: '10px'
                }}>+ Update pages</button>
              )}
            </div>
          ))}
            ...
            <>
              ...
              {finished.map(book => (
                <div key={book.id} style={{
                  ...
                  background: 'var(--surf)', border: '0.5px solid var(--border)',
                  borderRadius: '8px', marginBottom: '6px', alignItems: 'center', opacity: 0.7
                }}>
                  <div style={{
                    width: '30px', height: '42px', borderRadius: '4px',
                    background: 'var(--surf3)', flexShrink: 0,
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '7px' }}>
                <button onClick={addBook} style={{
                  flex: 1, background: 'var(--read)', border: 'none', borderRadius: '7px',
